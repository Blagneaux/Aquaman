...
void setup(){
  ...
  dat = new SaveData("saved/pressure.txt",circle.coords,(int)L,2*n,n,1);    // initialize the output data file with header information
  dat2 = new SaveData("saved/pressure.txt",circle.coords,(int)L,2*n,n,1);    // initialize the output data file with header information
}
void draw(){
  if (flow.QUICK) {
            dt = flow.checkCFL();
  ...
  if (posX < _n) {
    circle.follow();
    circle.translate(-(speed + Av*sin(2*PI*Fv*t))*dt,2*PI*Fy*Ay*cos(2*PI*Fy*t)*dt);                      // translate the body in a straight line
  ...
  posX += (speed + Av*sin(2*PI*Fv*t))*dt;
