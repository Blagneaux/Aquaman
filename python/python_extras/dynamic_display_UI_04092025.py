...
class CsvTailer:
    ...
    def wait_for_header(path, poll=0.1, timeout=10.0):
        """
        Block until the CSV header line exists and ends with a newline.
        ...
        while True:
            try:
                ...
                with open(path, "r", newline="", encoding="utf-8") as f:
                    line = f.readline()
                    if line.endswith("\n"):
                        _ = next(csv.reader([line]))  # parses -> header exists
                        return
            except Exception:
                pass
            if time.time() - start > timeout:
                raise TimeoutError(f"Header not available yet for {path}")
            time.sleep(poll)
...
def update(_frame, tailer, state, sc, title, DURATION):
...
def main(DURATION, PATH):
    """
    Run the UI with a state displayed every DURATION interval.
    Data is read at PATH, and only a window worth 2 DURATION of data is kept
    in memory.
    """
    ...
    ani = FuncAnimation(fig, update, fargs=(tailer, state, sc, title, DURATION), interval=DURATION, blit=False, cache_frame_data=False)
    plt.show()
