...
def parse_fixed_params(value: str) -> dict[str, float | int]:
    ...
    if len(parts) != 6:
        raise ValueError("fixed params must be 'hidden,hidden2,lr,weight_decay,epochs,batch_size'")
    hidden = int(float(parts[0]))
    hidden2 = int(float(parts[1]))
    lr = float(parts[2])
    wd = float(parts[3])
...
def train_torch_model(
    ...
    model = SimpleMLP(input_dim, hidden_size, hidden2_size, activation).to(device)
...
def plot_learning_and_confusion(
    ...
    ax_loss.set_title(
        f"{dataset} learning curve (h1={result.hidden_size}, h2={result.hidden2_size}, lr={result.lr})"
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train a nonlinear classifier on pressure snapshots.")
    parser.add_argument(
        "--snapshot-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default="0,4,8",
        help="Comma-separated second hidden sizes (0 disables second layer)",
    )
    parser.add_argument(
        "--batch-sizes",
        type=str,
    ...
    parser.add_argument(
        "--fixed-params",
        type=str,
        default="8,0,0.0005,0.0001,100,16",
        help="Fixed hyperparameters as 'hidden,hidden2,lr,weight_decay,epochs,batch_size'",
    )
    parser.add_argument(
        "--spike-ratio",
        type=float,
    ...
    hidden_sizes = parse_int_list(args.hidden_sizes, [4, 8, 16])
    hidden2_sizes = parse_int_list(args.hidden2_sizes, [0, 4, 8])
    ...
    for dataset in datasets:
        ...
        if args.skip_optuna:
            ...
            def objective(trial: optuna.Trial) -> float:
                ...
                hidden2_size = trial.suggest_categorical("hidden2_size", hidden2_sizes)
        ...
        best_outcome = train_torch_model(
            ...
            hidden2_size=int(best_params["hidden2_size"]),
            activation=args.activation,
        ...
        print(
            f"✅ {dataset}: samples={len(y)} | best h1={best_outcome.result.hidden_size}, "
            f"h2={best_outcome.result.hidden2_size}, lr={best_outcome.result.lr:.4g}, "
        ...
        out_path = args.modes_dir / f"classifier_nonlinear_results_{dataset}.csv"
        if results:
            save_results(out_path, results)
        summary_path = args.modes_dir / "classifier_nonlinear_results_summary.csv"
        ...
        if args.plot_best_metrics and best_outcome.history is not None:
            ...
            if best_state is not None:
                model = SimpleMLP(
                    X_train.shape[1],
                    best_outcome.result.hidden_size,
                    best_outcome.result.hidden2_size,
                    args.activation,
                )
                model.load_state_dict(best_state)
                model.eval()
                with torch.no_grad():
                    raw_train = model(torch.from_numpy(X_train).float()).numpy()
                    raw_test = model(torch.from_numpy(X_test).float()).numpy()
                if args.loss == "mse":
                    train_scores = raw_train
                    test_scores = raw_test
                else:
                ...
                plot_learning_and_confusion(
                    ...
                    dataset,
                    best_outcome.result,
                    best_outcome.result.best_epoch,
                )

                weights_path = args.modes_dir / f"classifier_nonlinear_best_weights_{dataset}.pt"
                torch.save(
                    {
                        "state_dict": best_state,
                        "best_epoch": best_outcome.result.best_epoch,
                        "hidden_size": best_outcome.result.hidden_size,
                        "hidden2_size": best_outcome.result.hidden2_size,
