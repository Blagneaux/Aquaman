...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train a simple CNN classifier on pressure snapshots.")
    parser.add_argument(
        "--snapshot-dir",
        type=Path,
    ...
    for dataset in datasets:
        ...
        out_path = args.modes_dir / f"classifier_cnn_results_{dataset}.csv"
        if results:
            save_results(out_path, results)
        summary_path = args.modes_dir / "classifier_cnn_results_summary.csv"
        ...
        if args.plot_best_metrics and best_outcome.history is not None:
            ...
            if best_state is not None:
                ...
                weights_path = args.modes_dir / f"classifier_cnn_best_weights_{dataset}.pt"
                torch.save(
                    {
