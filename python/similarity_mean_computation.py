...
for path in paths:
            ...
            for i in range(len(validity)):
                if validity[i] :#and file_name[-6:-4] != "31" and file_name[-6:-4] != "26": # 26 gives unrealistic values, probably due to the tail eating combined with the important size of the fish
                    ...
                    if channel[i] == "S1":
                        ...
                        Frechet1.append(frechet_dist[i])
                    elif channel[i] == "S2":
                        DTW2.append(dtw_score[i])
                        Frechet2.append(frechet_dist[i])
                    elif channel[i] == "S4":
                        DTW4.append(dtw_score[i])
                        Frechet4.append(frechet_dist[i])
                    # if index == 53:
    ...
    print("Mean Frechet distance for sensor 1:", np.mean(Frechet1))
    print("Mean Frechet distance for sensor 2:", np.mean(Frechet2))
    print("Mean Frechet distance for sensor 4:", np.mean(Frechet4))
